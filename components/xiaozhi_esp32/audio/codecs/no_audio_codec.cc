...
NoAudioCodecSimplex::NoAudioCodecSimplex(int input_sample_rate, int output_sample_rate, gpio_num_t spk_bclk, gpio_num_t spk_ws, gpio_num_t spk_dout, gpio_num_t mic_sck, gpio_num_t mic_ws, gpio_num_t mic_din, gpio_num_t pa_ctrl) {
    duplex_ = false;
    input_sample_rate_ = input_sample_rate;
    output_sample_rate_ = output_sample_rate;
    ...
    if (pa_ctrl != GPIO_NUM_NC) {
        gpio_config_t spk_ctrl_config = {
            .pin_bit_mask = 1ULL << pa_ctrl,
            .mode = GPIO_MODE_OUTPUT,
        };
        gpio_config(&spk_ctrl_config);
        gpio_set_level(pa_ctrl, 0);
        this->pa_ctrl = pa_ctrl;
    }
